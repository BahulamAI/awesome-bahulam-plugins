 ...
 * Same tool name + params across local and hosted substrates. This OSS
 * implementation writes the scene to local disk and returns the manim CLI
 * command for the agent to launch via `shell run_in_background`. A hosted
 * implementation can expose the same contract through MCP/Context Forge and
 * return a managed render URL. Both write the same manifest.json shape (see
 * config/manifest.schema.json).
 *
 * Layout (this file writes into .bahulam/tmp/content-creation/ under
 ...
 * checkout owns its own content-creation/ folder. Assets are optional; the
...
export async function call(args = {}, options = {}) {
  ...
  const root = path.join(process.cwd(), '.bahulam', 'tmp', 'content-creation');
