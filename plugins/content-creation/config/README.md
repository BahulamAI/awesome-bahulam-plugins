...
of content-creation: OSS CLI plugin and hosted SaaS execution through
MCP/Context Forge.
...
| `agents/*.yaml` | Copy, image, video, Three.js, planner, animator, and reviewer prompts | CLI + SaaS agent loaders |

## Tool ownership

Agent `tools:` arrays are allowlists. Content and Manim tools live in the
plugin root `tools/` directory:

- `content_brief` → `tools/content-brief.mjs`
- `record_asset` → `tools/record-asset.mjs`
- `write_threejs_scene` → `tools/write-threejs-scene.mjs`
- `list_assets` → `tools/list-assets.mjs`
...
`delegate`, `read_file`, `write_file`, `list_files`, `generate_image`,
`analyze_image`, `shell`, and `job_output`.
...
authored here (plugin.yaml + config/workspace.yaml + config/agents/)
        ...
        │      copies whole plugin dir to ~/.bahulam/plugins/content-creation/
        │      → CLI loads plugin.yaml, config.workspace, and config.agents_from
        │
        └── hosted SaaS/mobile substrate reads the same manifest
               → tools are exposed through MCP/Context Forge or an isolated
                 adapter with the same tool names and prompts
```
...
`config` block. Content Creation keeps subagents in `config/agents/*.yaml`
